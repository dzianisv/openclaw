...
const log = createSubsystemLogger("auto-reply/reply-media");
  ...
  return async (payload) => {
    ...
    for (const media of mediaList) {
      ...
      try {
        ...
        // A dropped reply media path is customer-visible (the reply appends
        // REPLY_MEDIA_FAILURE_WARNING below), so this must not be verbose-only.
        log.warn(`dropping blocked reply media ${media}`, {
          mediaPath: media,
          error: err,
        });
        continue;
