...
describe("proxy-ingress handler", () => {
  ...
  beforeEach(() => {
    ...
    installRuntime();
    handler = createProxyIngressHandler({} as never);
  });
  ...
  it("rejects updates without update_id", async () => {
    ...
    expect(handled).toBe(true);
    expect(result().status).toBe(400);
    expect(result().body).toContain("update_id");
  });
  ...
  it("returns before a long-running handleUpdate finishes", async () => {
    let resolveUpdate: (() => void) | undefined;
    botHandleUpdateMock.mockImplementationOnce(async () => {
    ...
    const update = { update_id: 11, message: { chat: { id: 123 }, text: "slow" } };
    const { req, write } = makeReq("POST", "/api/channels/telegram/proxy-ingress", update);
    const { res, result } = makeRes();
    write();
  ...
  it("keeps the HTTP ack successful when background handleUpdate throws", async () => {
  ...
  it("rejects invalid JSON body", async () => {
    ...
    await handler(req, res);
    expect(result().status).toBe(400);
    const payload = JSON.parse(result().body) as { error?: { message?: string } };
    expect(payload.error?.message).toBeTypeOf("string");
    expect((payload.error?.message ?? "").toLowerCase()).toContain("json");
  });
});
